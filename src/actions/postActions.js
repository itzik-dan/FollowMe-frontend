...
export const createPost =
  (title, body, photo) => async (dispatch, getState) => {
    try {
      dispatch({
...
export const listPosts = () => async (dispatch) => {
  try {
    ...
    const { data } = await axiosInstance.get("/allpost");
...
export const listFollowingPosts = () => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axiosInstance.get("/followingPosts", {
      headers: {
        Authorization: `Bearer ${userInfo.token}`,
      },
    });
...
export const deletePost = (postId) => async (dispatch, getState) => {
  try {
    ...
    await axiosInstance.delete(`/deletepost/${postId}`, config);
